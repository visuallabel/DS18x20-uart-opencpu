...
Enum_SerialPort m_port;

uint8_t ow_init(/*char *dev_path*/ Enum_SerialPort port)
{
	m_port = port;
	if (uart_init(port) != UART_SUCCESS)
		return OW_ERR_UART;
	else
...
uint8_t ow_rom_search(uint8_t diff, uint8_t *id)
{
	...
	do
	{
		...
		do
		{
			b = ow_bit(1); /* read bit */
			if (ow_bit(1))
			{ /* use brackets to tell the compiler the following else is mine */
				if (b)
